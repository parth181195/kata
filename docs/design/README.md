...
- **Pending until the Google OAuth clients exist** (Android client: package `dev.parth.kata`, debug SHA-1 `2B:CA:26:E4:4D:8C:D9:E8:F2:43:22:E5:CE:05:D0:F7:34:FE:31:C0`; Web client ID → VM `/opt/kata/api/.env` `GOOGLE_WEB_CLIENT_ID` + `pm2 reload kata-api --update-env`, and the build `--dart-define=KATA_GOOGLE_WEB_CLIENT_ID=<id>`): real sign-in → 340 seeded recipes → airplane-mode banner → RETRY → write to the X-S20 → Profile shows the Google account → sign out.
