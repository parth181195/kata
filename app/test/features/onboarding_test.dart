...
void main() {
  testWidgets('a first sign-in asks the two questions and opens the library on that sensor', (t) async {
    ...
    expect(find.text('WHICH BODY DO YOU SHOOT?'), findsOneWidget, reason: 'onboarding comes before the library');
    ...
    expect(c.read(libraryFilterProvider).sensor, 'X-Trans V');
  ...
  testWidgets('skipping leaves the library unfiltered and is remembered', (t) async {
    ...
    await t.pumpAndSettle();
    expect(find.text('Search recipes, film sims, authors'), findsOneWidget);
    expect(c.read(libraryFilterProvider).sensor, isNull);
  ...
  testWidgets('someone who already answered goes straight to the library', (t) async {
    final c = await pumpKata(t, signedIn: true, preferences: {'sensor': 'X-Trans IV', 'body': 'X-T4', 'onboardedAt': '2026-08-20T10:00:00.000Z'});
    expect(find.text('WHICH BODY DO YOU SHOOT?'), findsNothing);
    expect(find.text('Search recipes, film sims, authors'), findsOneWidget);
    expect(c.read(libraryFilterProvider).sensor, 'X-Trans IV');
