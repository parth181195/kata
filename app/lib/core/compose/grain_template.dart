...
class GrainTemplate {
  ...
  /// Deterministic: same (seed, grainPx) → identical bytes.
  /// [grainPx] is the clump size in physical pixels; the shader samples 1:1.
  static Float64List field({required int seed, required double grainPx}) {
    ...
    final fine = _blur(white, grainPx * 0.55);
    final coarse = _blur(white, grainPx * 1.6);
  ...
  static Future<ui.Image> image({required int seed, required double grainPx}) async {
    final f = field(seed: seed, grainPx: grainPx);
