...
class _LibraryScreenState extends ConsumerState<LibraryScreen> {
  ...
  void _setFilter(LibraryFilter Function(LibraryFilter) f) =>
      ref.read(libraryFilterProvider.notifier).update(f);

  Future<void> _pickFilmSim() async {
    final current = ref.read(libraryFilterProvider).filmSim;
    final picked = await showKataSheet<String?>(
      context,
      builder: (c) => KataSheet(
        eyebrow: 'Filter',
        ...
        children: [
          Wrap(
            ...
            runSpacing: 8,
            children: [
              for (final s in OfrEnums.filmSims)
                KataChip(
                  label: s,
                  selected: s == current,
    ...
    _setFilter(
      (f) => picked.isEmpty
          ? f.copyWith(clearFilmSim: true)
          : f.copyWith(filmSim: picked),
    );
  }
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  Row(
                    children: [
                      ...
                      _LayoutToggle(
                        layout: layout,
                        onChanged: (l) =>
                            ref.read(libraryLayoutProvider.notifier).set(l),
                      ),
                      ...
                      Material(
                        ...
                        child: InkWell(
                          onTap: () async {
                            ...
                            final pick = await showKataMenu<LibrarySort>(
                              context,
                              position: pos,
                              items: [
                                for (final (s, label) in [
                                  (LibrarySort.newest, 'Newest first'),
                                  (LibrarySort.popular, 'Most saved'),
                                  (LibrarySort.az, 'A → Z'),
                                ])
                                  KataMenuItem(
                                    s,
                            ...
                            if (pick != null) {
                              _setFilter((f) => f.copyWith(sort: pick));
                            }
                          },
                          child: Container(
                            height: 36,
                  ...
                  KataSearchField(
                    hint: 'Search recipes, film sims, authors',
                    height: KataSearchField.touch,
                    controller: _search,
                    onChanged: (q) => _setFilter((f) => f.copyWith(query: q)),
                  ),
                  const SizedBox(height: 12),
                  SingleChildScrollView(
                    ...
                    child: Row(
                      children: [
                        ...
                        KataChip(
                          label: 'B&W',
                          selected: filter.mono == true,
                          onTap: () => _setFilter(
                            (f) => f.mono == true
                                ? f.copyWith(clearMono: true)
                                : f.copyWith(mono: true),
                          ),
                        ),
                        const SizedBox(width: 7),
                        KataChip(
                          label: filter.filmSim ?? 'Film sim',
                          selected: filter.filmSim != null,
                          onTap: _pickFilmSim,
                          onRemove: filter.filmSim == null
                              ? null
                              : () => _setFilter(
                                  (f) => f.copyWith(clearFilmSim: true),
                                ),
                        ),
                        const SizedBox(width: 7),
                        KataChip(
                          label: filter.advancedCount == 0
                              ? 'Filters'
                              : 'Filters · ${filter.advancedCount}',
                    ...
                    children: [
                      Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          border: Border.all(color: p.dim, width: 1.5),
                        ),
                      ),
                      const SizedBox(width: 10),
                      Expanded(
                        child: Text(
                          lib.offlineIsNetwork
                              ? 'Offline — showing cached library'
                              : 'Library unreachable — showing cached copy',
                          maxLines: 1,
            ...
            Expanded(
              ...
              child: !lib.loaded || (lib.syncing && lib.all.isEmpty)
                      ...
                      child: KataCard(
                        ...
                        child: lib.all.isEmpty && lib.offline
                            ? KataEmptyState(
                                ...
                                body: lib.offlineIsNetwork
                                    ? 'Connect to the internet once to load the library.'
                                    : 'The library server is unreachable right now.',
                                actionLabel: 'Retry',
                                onAction: () => lib.sync(),
                              )
                            // The sync failed outright (the server said no, not "no results"):
                            // say that, instead of blaming the filters.
                            : lib.all.isEmpty && lib.syncError != null
                            ? KataEmptyState(
                                glyph: '!',
                            ...
                            : KataEmptyState(
                                ...
                                title: filter.query.isEmpty
                                    ? 'No katas match'
                                    : 'No katas for “${filter.query}”',
                                body:
                                    'Try a film sim (Classic Neg) or clear the Verified filter.',
                  ...
                  : RefreshIndicator(
                      ...
                      strokeWidth: 2,
                      onRefresh:
                          lib.refresh, // returns once the first page lands
        ...
        children: [
          seg(
            ...
            Container(
              ...
              decoration: BoxDecoration(
                border: Border.all(
                  color: layout == LibraryLayout.hero ? p.bg : p.muted,
                  width: 1.5,
                ),
            ...
            SizedBox(
              ...
              child: GridView.count(
                ...
                children: [
                  for (var i = 0; i < 4; i++)
                    ColoredBox(
                      color: layout == LibraryLayout.grid ? p.bg : p.muted,
                    ),
                ],
              ),
            ),
